...
<template>
  <div class="cov-vue-date" :class="option.wrapperClass ? option.wrapperClass : {}">
    <div class="datepickbox">
      ...
      <i class="fa fa-calendar" @click="showCheck"></i>
    </div>
    <div class="datepicker-mask" v-if="showInfo.check" @click="dismiss($event)"></div>
    <div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)" style="background:transparent">
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
       ...
       <!--      <br> -->
        ...
        <div class="cov-date-box list-box" v-if="showInfo.hour">
          <div class="cov-picker-box date-list">
            <div class="date-picker-tit">
              <p class="date-list--tit">时</p>
              <p class="date-list--tit">分</p>
              <p class="date-list--tit">秒</p>
            </div>
            <div class="watch-box">
              <div class="hour-box">
              ...
              <div class="second-box">
                <div class="mui-pciker-rule mui-pciker-rule-ft"></div>
                <div class="second-item" v-for="sitem in seconds" @click="setTime('second',sitem, seconds)" :class="{'active':sitem.checked}">{{sitem.value}}</div>
              </div>
            </div>
          </div>
        </div>
        <div class="button-box" v-if="dateMulti||option.buttons">
          <span @click="clear" v-if="option.buttons&&option.buttons.clear">{{option.buttons? option.buttons.clear : 'Clear' }}</span>
          <span @click="showInfo.check=false" v-if="dateMulti||option.buttons.cancel">{{option.buttons? option.buttons.cancel : '取消' }}</span>
          <span @click="picked" v-if="dateMulti||option.buttons.ok">{{option.buttons? option.buttons.ok : '确定'}}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  exports.default = {
    ...
    methods: {
      ...
      getLimitCondition: function getLimitCondition(limit, day) {
        var tmpMoment = day.moment;//(0, _moment2.default)(this.checked.year + '-' + this.pad(this.checked.month) + '-' + this.pad(day.value));
        if (limit.from && !limit.to) {
      ...
      checkDay: function checkDay(obj) {
        ...
        if (this.option.type === 'day' || this.option.type === 'second') {
      ...
      picked: function picked() {
        // if (this.option.type === 'day' || this.option.type === 'min') {
        if (this.option.type === 'day' || this.option.type === 'second') {
          var ctime = this.checked.year + '-' + this.checked.month + '-' + this.checked.day + ' ' + this.checked.hour + ':' + this.checked.min + ':' + this.checked.second;
          this.checked.currentMoment = (0, _moment2.default)(ctime, 'YYYY-MM-DD HH:mm:ss');
      ...
      dismiss: function dismiss(evt) {
        if (evt.target.className === 'datepicker-mask') {
      ...
      shiftActTime: function shiftActTime() {
        ...
        this.$nextTick(function () {
          ...
          document.querySelector('.second-box').scrollTop = (document.querySelector('.second-item.active').offsetTop || 0) - 250;
        });
      }
    }
  };
